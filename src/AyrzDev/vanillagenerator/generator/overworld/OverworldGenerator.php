...
namespace AyrzDev\vanillagenerator\generator\overworld;

use AyrzDev\vanillagenerator\generator\Environment;
use AyrzDev\vanillagenerator\generator\ground\DirtAndStonePatchGroundGenerator;
use AyrzDev\vanillagenerator\generator\ground\DirtPatchGroundGenerator;
use AyrzDev\vanillagenerator\generator\ground\GravelPatchGroundGenerator;
use AyrzDev\vanillagenerator\generator\ground\GroundGenerator;
use AyrzDev\vanillagenerator\generator\ground\MesaGroundGenerator;
use AyrzDev\vanillagenerator\generator\ground\MycelGroundGenerator;
use AyrzDev\vanillagenerator\generator\ground\RockyGroundGenerator;
use AyrzDev\vanillagenerator\generator\ground\SandyGroundGenerator;
use AyrzDev\vanillagenerator\generator\ground\SnowyGroundGenerator;
use AyrzDev\vanillagenerator\generator\ground\StonePatchGroundGenerator;
use AyrzDev\vanillagenerator\generator\noise\glowstone\PerlinOctaveGenerator;
use AyrzDev\vanillagenerator\generator\noise\glowstone\SimplexOctaveGenerator;
use AyrzDev\vanillagenerator\generator\overworld\biome\BiomeHeightManager;
use AyrzDev\vanillagenerator\generator\overworld\biome\BiomeIds;
use AyrzDev\vanillagenerator\generator\overworld\populator\OverworldPopulator;
use AyrzDev\vanillagenerator\generator\overworld\populator\SnowPopulator;
...
use AyrzDev\vanillagenerator\generator\overworld\biome\Biome3DGenerator;
use AyrzDev\vanillagenerator\generator\cave\CaveGenerator;
...
class OverworldGenerator extends VanillaGenerator
{
	...
	private ?CaveGenerator $cave_generator = null;
	...
	protected function generateChunkData(ChunkManager $world, int $chunk_x, int $chunk_z, VanillaBiomeGrid $grid): void
	{
		...
		$min_y = $world->getMinY();
		$max_y = $world->getMaxY();
		Biome3DGenerator::generate3DBiomes($grid, $this->random, $chunk_x, $chunk_z, $min_y, $max_y);

		for ($x = 0; $x < $size_x; ++$x) {
			for ($z = 0; $z < $size_z; ++$z) {
				$surface_biome_id = $grid->getBiome($x, $z);
				for ($y = $min_y; $y < $max_y; ++$y) {
					$biome_3d = $grid->getBiome3D($x, $y, $z);
					if ($biome_3d !== null) {
						$chunk->setBiomeId($x, $y, $z, $biome_3d);
					} else {
						// Fallback to surface biome if no 3D biome is set
						$chunk->setBiomeId($x, $y, $z, $surface_biome_id);
					}
				}
				if ($surface_biome_id !== null && array_key_exists($surface_biome_id, self::$GROUND_MAP)) {
					self::$GROUND_MAP[$surface_biome_id]->generateTerrainColumn($world, $this->random, $cx + $x, $cz + $z, $surface_biome_id, $surface_noise[$x | $z << Chunk::COORD_BIT_SIZE]);
				} else {
					$this->ground_gen->generateTerrainColumn($world, $this->random, $cx + $x, $cz + $z, $surface_biome_id, $surface_noise[$x | $z << Chunk::COORD_BIT_SIZE]);
				}
			}
		}
		// Caves: carve after base stone terrain and biome assignment (legacy)
		if ($this->cave_generator !== null) {
	...
	protected function createWorldOctaves(): WorldOctaves
	{
		...
		$roughness2 = PerlinOctaveGenerator::fromRandomAndOctaves($seed, 16, 5, 48, 5);
		$roughness2->x_scale = self::COORDINATE_SCALE;
		$roughness2->y_scale = self::HEIGHT_SCALE;
		$roughness2->z_scale = self::COORDINATE_SCALE;

		$detail = PerlinOctaveGenerator::fromRandomAndOctaves($seed, 8, 5, 48, 5);
			...
			for($j = 0; $j < 5 - 1; ++$j){
				for($k = 0; $k < 48 - 1; ++$k){
					...
					for($l = 0; $l < 8; ++$l){
						...
						$y_pos = max(-64, min(319, $y_pos));
						
						$y_block_pos = $y_pos & 0xf;
						// Calculate subchunk index: Y=-64 maps to subchunk -4, Y=319 maps to subchunk 19
						$subchunk_index = $y_pos >> Chunk::COORD_BIT_SIZE;
						...
						$sub_chunk = $chunk->getSubChunk($subchunk_index);
